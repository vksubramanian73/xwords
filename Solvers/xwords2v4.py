...
import time
# 99.9 (BIGGEST MEME CHECK HAHA)
# lowk random but changes made:
# neighbors: checking how many words have letter replacing star (word[space]), and sorting based on that
...
def placeLetters(puzzle, wordsUsed):
    ...
    if space == -1: return ''
    if height == width == 15 or height == width == 5: print(strPuzzle(puzzle)); print()
...
def findLetters(puzzle, wordsUsed, space):
    ...
    vLetters = {}; hLetters = {}
    if vLetterIndex == -1: vLetters = {i:26 for i in gLETTERS}
    else:
    ...
    if hLetterIndex == -1: hLetters = {i:26 for i in gLETTERS}
    else:
    ...
    letters = {}
    for i in hLetters:
        if i in vLetters: letters[i] = hLetters[i] + vLetters[i]
